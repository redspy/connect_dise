...
- **흔들기/던지기 같은 모션 제스처는 `SensorManager`가 이미 계산해주는 `shakeMagnitude`를 쓸 것, `onMotion` 콜백의 원본 `acc`(가속도)로 직접 재계산하지 말 것**: `platform/client/shared/SensorManager.js`의 `onMotion`은 `{ shakeMagnitude, acc }`를 콜백에 넘기는데, `shakeMagnitude`는 이미 중력(~9.8m/s²)을 뺀 "실제 움직임" 크기입니다. `acc`(원본 `accelerationIncludingGravity`)로 `Math.max(|x|,|y|,|z|)` 같은 걸 직접 계산하면 정지 상태에서도 중력이 어느 한 축에 그대로 실려 임계값을 상시 통과해버려서, 폰이 가만히 있어도 흔들림 애니메이션/진동이 끊임없이 발동하는 버그가 생깁니다(주사위 게임에서 합성 devicemotion 이벤트(x=0,y=0,z=9.8, 완전 정지)로 실측 확인 — spin-battle은 처음부터 `shakeMagnitude`를 올바르게 쓰고 있었음, 2026-08-20).
- **`<game-appbar>`는 반드시 HTML 태그로 배치하고 `document.querySelector('game-appbar').onRestart = fn;`으로 연결할 것 — `new AppBar(id, opts)`처럼 클래스를 직접 생성자 호출하지 말 것**: `AppBar`(`platform/client/shared/AppBar.js`)는 `customElements.define('game-appbar', AppBar)`로 등록된 Web Component이고, 커스텀 `constructor()`가 없어서 `new AppBar('game-appbar', {title, onRestart})` 같은 인자는 전부 조용히 무시됩니다. 게다가 이렇게 생성한 인스턴스는 DOM에 붙지 않아 `connectedCallback()`(실제 렌더링)이 아예 실행되지 않습니다 — 즉 뒤로가기/제목/다시하기 버튼이 통째로 화면에 나타나지 않는데도 콘솔 에러 하나 없이 조용히 실패합니다(dice 게임에서 처음부터 이 상태였던 것으로 실측 확인, 2026-08-20). 반드시 `<game-appbar title="..."></game-appbar>`를 HTML에 두고, JS에서는 `document.querySelector('game-appbar').onRestart = () => this.resetSession();`로만 다시하기 콜백을 연결할 것 — `HostBaseGame.js`가 이미 `AppBar.js`를 import해 커스텀 엘리먼트를 등록해주므로 게임 쪽에서 별도 import도 불필요합니다.
- **`body.host-board`는 플랫폼 공통으로 `display:flex; justify-content:center; align-items:center;`임 — 장식용 `position:absolute`/`fixed` 요소에 무조건 `position:relative`를 주입하지 말 것**: 4코너 QR처럼 `position:absolute`(또는 반응형 미디어쿼리의 `position:static`)로 문서 흐름 밖에 있던 요소에 JS가 (예: 오버레이 자식을 위한 포지셔닝 컨텍스트 확보 목적으로) `el.style.position = 'relative'`를 조건 없이 걸면, 그 순간 그 요소가 `body.host-board`의 flex-row 흐름에 실제로 편입돼 중앙 로비 카드를 옆으로 밀어내며 레이아웃이 붕괴합니다(주사위 게임의 데모 모드에서 QR 블러 오버레이를 씌울 때 `container.style.position='relative'`를 무조건 실행해 재현, 2026-08-20). `getComputedStyle(el).position === 'static'`일 때만 `relative`로 바꾸고, 원상복구 시에는 `el.style.position = ''`로 반드시 되돌릴 것.
- **데모 배너 높이 CSS 변수 오프셋은 `padding-top`뿐 아니라, 하드코딩된 `top` 값을 가진 `position:absolute` 요소에도 별도로 `calc(기존값 + var(...))`를 적용해야 함**: `padding-top`은 절대위치 자식의 기준(포함 블록)인 "패딩 박스"의 안쪽에 있는 값이라, 조상에 `padding-top`을 걸어도 그 조상의 패딩 박스 자체는 이동하지 않으므로 `position:absolute; top:60px;`인 자식은 전혀 밀려나지 않습니다. 4코너 QR처럼 좌표가 하드코딩된 요소는 `#qr-top-left, #qr-top-right { top: calc(60px + var(--demo-banner-h, 0px)); }`처럼 그 요소 자신의 `top`에 직접 더해야 배너가 커져도 가려지지 않습니다(주사위 게임에서 `padding-top`만 걸었다가 QR이 전혀 안 밀리는 것을 실측한 뒤 수정, 2026-08-20).
- **좁은/짧은 화면용 반응형 미디어쿼리에서 `justify-content:center`인 flex 컨테이너에 콘텐츠(배너 오프셋 등)를 추가해 총 높이가 뷰포트를 넘기면, 위쪽이 스크롤로 닿지 않는 채 다음 요소와 겹쳐 보이는 버그가 생김**: `justify-content:center`는 넘치는 콘텐츠를 위/아래로 균등하게 넘치게 두는데, 위쪽 초과분은 `overflow-y:auto`로도 스크롤해서 볼 수 없어 그냥 다음 요소와 겹친 것처럼 보입니다. 콘텐츠 총량이 뷰포트보다 커질 수 있는 반응형 구간(특히 데모 배너처럼 동적으로 높이가 늘어나는 경우)에서는 `justify-content: flex-start`로 바꿔 위에서부터 쌓이고 자연스럽게 스크롤되도록 할 것(주사위 게임 900×700 데모 모드에서 실측 확인, 2026-08-20).
- **`HostBaseGame`의 60초 유휴 자동 데모(`_triggerAutoDemo`)는 로비에 이미 실제 플레이어가 있으면 절대 시작하면 안 됨**: 파티에서 다들 폰으로 닉네임 입력 중이라 호스트 PC 화면만 60초간 아무도 안 건드리는 상황이 흔한데, 이때 자동 데모가 끼어들면 대기 중이던 실제 플레이어 준비 상태와 무관하게 봇들이 로비를 채우고 게임이 강제로 시작돼버립니다. 플랫폼 레벨(`platform/client/HostBaseGame.js`의 `_triggerAutoDemo()`)에 `if (this._players.size > 0) return;` 가드를 추가해 모든 게임에 공통 적용했습니다(2026-08-20, dice 검수 중 claude 헤드리스 리뷰로 발견 — 게임별 `DemoSimulator.js`가 아니라 플랫폼 공통 코드의 결함이었음에 주의).
- **위 유휴 자동 데모 가드는 "60초 자동 트리거" 경로에만 적용되기 쉬우니, 게임 자체의 수동 "🤖 데모 플레이 실행" 버튼 클릭 핸들러에도 동일한 `if (this.playerCount > 0) return;` 가드를 반드시 별도로 넣을 것**: 자동 트리거를 플랫폼 공통 코드에서 막아뒀다고 해서 게임별 버튼 클릭 로직까지 안전해지는 게 아닙니다 — 로비에 실제 플레이어가 대기 중인데 호스트가 실수로(또는 시연 목적으로) 데모 버튼을 누르면, 봇 3명이 실제 플레이어와 뒤섞여 등록되고 `updateLobbyReady(3)`류의 하드코딩된 카운트가 실제 준비 인원 표시를 덮어쓰며, 곧바로 카운트다운까지 시작돼버립니다(give-you-fire 검수에서 claude 헤드리스 리뷰로 발견, 2026-08-21).
- **결과 화면 순위/점수를 집계하는 함수(`_buildFinalRankings()`류)는 `this.players`(현재 접속 중인 플레이어만 담긴 Map)가 아니라 게임 자체의 데이터 저장소(`_playerData` 등)를 순회할 것**: `HostBaseGame`은 플레이어가 완전히 퇴장(재접속 유예시간 만료 등)하면 게임의 `onPlayerLeave` 훅을 부르기도 전에 `this._players`에서 그 플레이어를 이미 지워버립니다. 게임 쪽 `onPlayerLeave`가 `_playerData`에 등수를 정확히 기록해도, 결과 집계 시점에 `this.players`를 기준으로 필터링하면 그 중도 이탈자가 최종 결과에서 통째로 사라집니다(give-you-fire 검수에서 claude 헤드리스 리뷰로 발견 — 2인 게임에서 한 명이 중간 퇴장하면 최종 순위가 1명짜리로 나오는 것으로 실측 확인, 2026-08-21). 게임 시작 시점에 각 플레이어의 색상 등 표시에 필요한 정보를 `_playerData`에 함께 스냅샷해 두면, 이후 `this.players`를 다시 조회할 필요가 없어 안전합니다.
- **데모 시뮬레이터가 `sdk._players`처럼 `HostBaseGame` 자신의 것이 아닌 SDK 내부 Map까지 직접 조작(mutate)하는 경우, 정리(cleanup)도 "봇 id만 골라 제거"해야지 데모 시작 전 스냅샷으로 통째로 되돌리면 안 됨**: `this.game._players` 하나에만 적용되는 줄 알았던 "전체 교체보다 골라서 추가/제거" 원칙(위 getter-only 항목 참고)은, 데모가 `this.game.sdk._players`처럼 SDK가 직접 관리하는 Map까지 손대는 구현이라면 그 Map에도 동일하게 적용해야 합니다. 데모 도중 실제 플레이어가 join하면 그 플레이어는 `sdk._players`에도 봇과 함께 섞여 들어가 있는데, `stopDemo()`가 이 Map을 데모 시작 이전 스냅샷으로 통째로 되돌리면 그 실제 플레이어까지 함께 지워집니다. 더 나쁜 건, 이후 `resetSession()`이 호출되면 `HostBaseGame`의 `reset` 핸들러가 `sdk.getPlayers()`(= `sdk._players`)로 `this.players`를 다시 채우기 때문에, 이 순간 그 실제 플레이어가 서버·본인 폰은 연결된 채로 착각하는데 호스트 화면에서만 완전히 증발하는 유령 상태가 됩니다(give-you-fire 검수에서 실측 확인, 2026-08-21).
- **공용 버튼 베이스 클래스(예: `.gyf-btn`)에 `flex: 1`이 있으면, 그 클래스를 물려받는 모든 신규 버튼이 예상 못 한 flex 컨테이너 안에서 통째로 늘어날 수 있음 — 여러 버튼이 나란히 폭을 나눠 갖는 용도(`display:flex` 액션 행)로 설계된 속성이므로, 그 버튼이 다른 요소와 `justify-content:space-between`으로 배치되는 등 "혼자 배치되는" 컨텍스트에서 재사용되면 남는 flex 공간을 전부 차지해 버튼이 거대하게 늘어납니다**: 특히 "small/danger" 같은 단독 배치용 variant 클래스는 재사용 시 반드시 `flex: none`을 명시적으로 지정할 것(give-you-fire의 헤더 "리셋" 버튼이 `.gyf-playing-header`의 `justify-content:space-between` 안에서 헤더 폭 대부분을 차지하도록 늘어난 것을 1920×1080/1280×400 인원 스트레스 테스트에서 실측 확인, 2026-08-21).
- **"호스트(보드판) 기기 자신도 플레이어로 참여시키고 싶다"는 요구는, 모바일 클라이언트 페이지를 호스트 페이지 안에 `<iframe src="/games/<id>/mobile/?session=<sessionId>">`로 그대로 임베딩하는 방식으로 해결할 것 — 호스트 게임 로직/SDK를 건드리지 말 것**: `HostSDK`가 QR에 인코딩하는 URL이 이미 이 형태(`HostSDK.js`의 `_qrUrl` 조립부)이고, 모바일 페이지는 그 쿼리파라미터 하나로 완전히 동작합니다. 이 URL을 iframe에 넣으면 그 iframe은 서버 입장에서 진짜 소켓 접속을 하는 진짜 플레이어가 되어(가짜/로컬 플레이어가 아님), 재접속 유예시간·데모 가드·`sendToPlayer`/`broadcast`·`onPlayerLeave`/`_buildFinalRankings()` 같은 기존 규칙이 전부 특수 처리 없이 그대로 적용됩니다. `TetrisMobile.js`의 `_bindGestureControls()`처럼 이미 마우스도 지원하는("데스크톱 테스트용" 주석) 모바일 컨트롤이면 PC 호스트에서도 그대로 조작 가능합니다. give-you-fire "간단대결모드"에서 이 패턴으로 구현해 실제 로비 참가→준비→게임 시작→마우스 드래그 조작→대시보드 반영→닫기 시 재접속 유예 배너까지 전부 기존 흐름 그대로 동작함을 Playwright로 검증(2026-08-21) — 호스트 자체를 특수 플레이어로 취급하는 코드는 한 줄도 필요 없었음.
- **"PC/TV 호스트 없이 폰만으로 방을 만들고 싶다"는 요구는, 위 iframe 임베딩 패턴을 뒤집어서 모바일 페이지가 숨겨진(`display:none`) host iframe을 자기 자신 안에 띄우는 방식으로 해결할 것**: `platform/client/HostBaseGame.js`의 `sessionReady` 핸들러에 `window.parent !== window`일 때 `postMessage({type:'platform:embeddedSessionReady', sessionId, qrUrl})`를 보내는 플랫폼 공통 브릿지를 추가해뒀습니다 — 어느 게임이든 자기 host 페이지를 숨김 iframe으로 띄우기만 하면 세션 생성 완료 시점을 알 수 있습니다. 받은 쪽은 `history.replaceState`로 `?session=`을 채우고 `MobileSDK.joinSession(sessionId)`(새로 추가된 공개 메서드 — 이미 연결된 소켓을 재사용해 페이지 새로고침 없이 참가)를 호출하면 그 페이지 자신도 완전히 평범한 플레이어가 됩니다. `MobileSDK` 생성자에 `{ onCreateRoom }` 콜백을 넘기면 원래 있던 "방에 연결하기" 모달(세션 없이 접속했을 때 자동으로 뜨는 공용 UI, 방 코드 입력/QR 스캔 기능이 이미 있었음)에 "🏠 새 방 만들기" 버튼이 추가로 노출됩니다. **주의**: `<game-lobby>`의 "게임 시작" 버튼은 host 페이지(이제 숨겨짐)에만 있어 아무도 못 누르게 되므로, 방을 만든 사람의 모바일 화면에 별도로 "게임 시작" 버튼을 만들어 `sendToHost('requestStart', {})` → host가 `onMessage('requestStart', ...)`로 받아 기존 `_canStart()`/`_startCountdown()`을 그대로 호출하게 연결해야 합니다(이미 있던 `requestRematch` 패턴과 동일 — give-you-fire에서 실제로 이 연결을 빼먹어 "전원 준비완료인데 아무도 시작을 못 누르는" 상태로 막힐 뻔한 것을 구현 중 자체 발견, 2026-08-22). 호스트 소켓이 끊기면 `SessionManager`가 유예 없이 세션을 즉시 종료하므로(플레이어와 비대칭), 방장 폰에는 `navigator.wakeLock.request('screen')`으로 화면 꺼짐을 방지하는 완화책을 권장.
- **`MobileSDK`의 "방에 연결하기" 폴백 모달(`_showFullReconnectUI()`, `?session=` 없이 접속했을 때 자동으로 뜨는 공용 UI)의 배경이 `rgba(0,0,0,0.85)`로 살짝 반투명해서, 뒤에 항상 렌더링돼 있는 게임 자체의 setup 화면(닉네임 입력 등 밝은 텍스트)이 15% 투과로 비쳐 보여 모달 문구와 겹쳐 읽히는 버그가 있었음**: 이 모달은 원래 "세션 없이 잘못 들어온 에러 상황"에서만 잠깐 보이는 것으로 취급돼 아무도 눈여겨보지 않았지만, 위 "폰으로 방 만들기" 패턴을 쓰면 이 모달이 의도된 정상 진입점이 되어 노출 빈도가 크게 늘어남 — `rgba(4, 6, 12, 0.98)`로 불투명도를 올려 해결(2026-08-22). Vite HMR로 이 파일을 고친 직후 재확인할 땐 브라우저가 이전 모듈을 잠깐 캐시해 옛 스타일로 보일 수 있으니, 새로고침 후 재스크린샷해서 진짜 반영됐는지 확인할 것.
- **모바일 클라이언트가 "op 하나를 보내고 그 결과(`opAck`로 비동기 전달되는 `newMeldId` 등)를 곧바로 다음 op에 써야 하는" 다단계 조작(조커 회수의 "대체 타일 삽입 → 조커 제거" 2단계, 세트 분할의 "여러 장을 새 세트로 순차 이동" N단계 등)을 구현할 때, 동기 for 루프나 고정 `setTimeout(fn, Nms)`으로 다음 op를 바로 쏘지 말 것**: `_sendMoveTile(...)`은 `boardOp`를 보내고 즉시 리턴하며, 실제 상태 반영(`_workBoard` 갱신, `newMeldId` 확정)은 나중에 별도로 도착하는 `opAck` 메시지 핸들러에서 일어난다. 두 번째 op가 첫 번째 op의 결과(방금 생긴 `newMeldId`, 또는 갱신된 `_workBoard`)를 참조해야 하는데 동기 루프로 곧바로 보내면 그 값이 아직 `undefined`/구버전 상태이고, 고정 지연 `setTimeout`으로 보내면 네트워크 지연이 그 값을 초과할 때만 가끔 실패하는 재현이 어려운 버그가 된다(루미큐브 조커 회수·세트 분할 두 곳에서 독립적으로 동일 패턴의 버그가 발견됨, 2026-08-23). 반드시 `_sendMoveTile(tileId, from, to, onAck)`의 네 번째 인자(성공 시 콜백)에 다음 op 호출을 넣어 콜백 체인으로 순서를 강제할 것 — N단계면 재귀 함수로 체이닝.
- **`onPlayerRejoin`/`requestState` 같은 재접속 핸들러를 `if (!this._gameStarted) return;`로만 가드하지 말 것 — 게임 종료 함수(`_endGame()` 등)가 그 안에서 `this._gameStarted = false`로 되돌리는 경우, "결과 화면 표시 중"이 정확히 이 가드에 걸려 재접속 응답 자체가 나가지 않는다**: 결과 화면은 흔히 몇 초~몇십 초간 떠 있고, 그 사이 플레이어가 새로고침하거나 네트워크가 잠깐 끊겼다 붙는 일이 실제로 생긴다. 이때 재접속 응답이 아예 안 오면 모바일 클라이언트는 (자체적으로도 결과를 재구성할 방법이 없어) "waiting" 같은 엉뚱한 화면에 얼어붙는다. 가드는 `if (!this._gameStarted && this.phase !== 'result') return;`처럼 결과 단계를 예외로 열어두고, `_endGame()` 시점에 종료 데이터(`{ endType, winnerIds, scores, revealedHands }`)를 인스턴스 필드(예: `_lastResult`)에 캐싱해 상태 동기화 페이로드에 실어 보낼 것 — `onReset()`에서 이 필드도 함께 `null`로 비워야 함. 모바일 쪽 상태 동기화 핸들러도 `phase === 'result'`일 때 단순히 대기 화면을 보여주고 끝내지 말고, 캐싱된 결과 데이터가 있으면 곧바로 결과 화면을 재구성해야 한다(루미큐브 검수에서 발견 및 실제 재접속 시나리오로 재현·수정 확인, 2026-08-23).
