...
2.  **2단계: 가상 전문가 병렬 분석 기동 (Parallel Multi-Agent Analysis)**
    *   PMOrchestrator의 오케스트레이션에 따라 메인 모델(Gemini), Codex(구조/문서 싱크), Claude(품질/아키텍처 누수) 에이전트를 가상 기동하여 원인 파악 및 병렬 분석을 개별적으로 수행합니다.
3.  **3단계: 분석 결과 취합 및 다각도 해결 설계 (Aggregation & Multi-perspective Design)**
    *   각 에이전트의 개별 피드백을 한곳에 합쳐 종합하고, 이를 해결하기 위해 각 모델이 수립한 수정 설계안의 장단점을 회의록의 대조 형태로 도출합니다.
4.  **4단계: 최종 수정 방향 확정 및 구현**
    *   회의를 거쳐 가장 아키텍처적으로 건전하고 안전한 단 하나의 해결 방향을 확정한 뒤에야 비로소 실물 소스 코드를 편집하고 E2E 검증을 적용합니다.
