...
게임 기획 및 개발 개발이 완료되면 스테이징(Stage) 및 커밋(Commit)하기 전에 반드시 다음 로컬 CLI 도구를 구동하여 품질을 정밀 검증해야 합니다.

1. **Claude CLI를 통한 코드 리뷰**:
   - 변경된 git diff 소스 코드를 바탕으로 로컬 `/Users/soul/.local/bin/claude` CLI를 구동하여 아키텍처 결함, 소켓 지터, 메모리 유수 여부에 대한 코드 리뷰를 필히 얻어야 합니다.
2. **Codex CLI를 통한 문서 정합성 싱크 검사**:
   - 개발 완료 문서(`AGENTS.md`, `walkthrough.md`, 회의록 등)와 실제 구현 코드 간 명세 일치 여부를 로컬 `/usr/local/bin/codex` CLI를 호출해 체크해야 합니다.
3. **가상 전문가 회의 피드백 반영**:
