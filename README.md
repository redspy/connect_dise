...
터미널에 출력된 주소 중 **`Network`** 주소(예: `https://192.168.x.x:5173/`)를 PC 브라우저에 입력하여 엽니다.  

> [!WARNING]
> **반드시 `Network` (로컬 IP) 주소를 사용하세요!**
> `localhost`로 접속하면 화면 사방에 생성되는 QR 코드 역시 `localhost` 주소를 포함하게 됩니다. 모바일 기기가 QR 코드를 스캔했을 때, 모바일 기기 입장에서는 자신의 `localhost`를 찾게 되어 접속이 불가능해집니다.
> 스마트폰 카메라로 QR 코드를 스캔하여 게임 방에 접속하려면, 메인 PC에서부터 호스트의 네트워크 IP(예: `192.168...`)가 적용된 주소로 열려야만 올바른 QR이 렌더링 됩니다.

```text
➜  Local:   https://localhost:5173/  (X 사용하지 마세요)
➜  Network: https://192.168.x.x:5173/  (O 이 주소로 리눅스/PC 접속)
```
